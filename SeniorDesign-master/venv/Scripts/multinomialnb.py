...
for filename in glob.glob(os.path.join(file_path, '*.txt')):
    with open(filename, 'r', encoding = "ISO-8859-1") as infile:
        whitepapers.append(infile.read())
        labels.append(1)
...
cleaned_whitepapers = clean_text(whitepapers)
term_docs = cv.fit_transform(cleaned_whitepapers)
print(term_docs [0])
...
cleaned_test = clean_text(whitepapers_test)
...
X_train, X_test, Y_train, Y_test = train_test_split(cleaned_whitepapers, labels, test_size=0.33, random_state=42)
...
cleaned_whitepapers_np = np.array(cleaned_whitepapers)
...
for train_indices, test_indices in k_fold.split(cleaned_whitepapers, labels):
    X_train, X_test = cleaned_whitepapers_np[train_indices], cleaned_whitepapers_np[test_indices]
